...
class SimulationTools {
  ...
  public:
    ...
    void SetRestoredHist( std::vector<TH3D*> &histRestored );
    void SetMeasuredHist( std::vector<TH3D*> &histMeasured );
...
int SimulationTools::Init()
{
  ...
  for( int ipid=0; ipid<nParticleType; ipid++ ) 
  {
    ...
    for( int ipt=0; ipt<nBinPt; ipt++ )
      for( int irap=0; irap<nBinRap; irap++ ) 
        hist_phi_diff[ipid][ipt][irap] = new TH2D( Form("hist_phi_diff_%s_p%d_y%d",particleName[ipid].Data(),ipt,irap) , "Blurring Matrix; #phi; #phi'", nBinPhi, phiMin, phiMax, nBinPhi, phiMin, phiMax );
  }
  ...
  if( fRLfile )
  {
    ...
    for( int ipid=0; ipid<nParticleType; ipid++ )
    {
      //hist_pt_rap_phi_real[ipid] = (TH3D*) fRLfile->Get( Form("hist_pt_rap_phi_corr_%s", particleName[ipid].Data()) );
      hist_pt_rap_phi_real[ipid] = (TH3D*) fRLfile->Get( Form("hist_pt_rap_phi_real_%s", particleName[ipid].Data()) );
      hist_pt_rap_phi_meas[ipid] = (TH3D*) fRLfile->Get( Form("hist_pt_rap_phi_meas_%s", particleName[ipid].Data()) );
      hist_pt_rap_phi_rest[ipid] = (TH3D*) fRLfile->Get( Form("hist_pt_rap_phi_rest_%s", particleName[ipid].Data()) );
      fHistogramMeasured.push_back( hist_pt_rap_phi_meas[ipid] ); // This should be okay...
      fHistogramRestored.push_back( hist_pt_rap_phi_rest[ipid] ); // This should be okay...
    }
    ...
    if( !bUseOptimal )
    {
      for( int ipid=0; ipid<nParticleType; ipid++ )
        for( int ipt=0; ipt<nBinPt; ipt++ )
          for( int irap=0; irap<nBinRap; irap++ ) hist_phi_diff[ipid][ipt][irap] = (TH2D*) fRLfile->Get( Form("hist_phi_diff_%s_p%d_y%d",particleName[ipid].Data(),ipt,irap) );
    }
  ...
  if( fHistogramRestored.size() != 0 )
  {
    ...
    for( int ipid=0; ipid<nParticleType; ipid++ )
    {
      for( int ipt=0; ipt<nBinPt; ipt++ )
      {
        ...
        for( int irap=0; irap<nBinRap; irap++ )
        {
          for( int iphi=startBin; iphi<=endBin; iphi++ ) // The both end bins are the same bin
          {
            int thisBin = fHistogramRestored[ipid]->GetBin( ipt+1, irap+1, iphi+1 );
            double binContent = fHistogramRestored[ipid]->GetBinContent( ipt+1, irap+1, iphi+1 );
...
void SimulationTools::CreateTheFirstData()
{
  ...
  for( int iEvent=0; iEvent<fNumberOfEvents; iEvent++ ) // loop over events
  {
    ...
    for( int iTrack=0; iTrack<nTracks; iTrack++ )
    {
      ...
      if( fModel ) 
      {
        if( bUseOptAzi )
        {
          int iBinPhi = TMath::Nint( particle_phi/binWidthPhyOpt[iBinPt-1] );
          double val = hist_pt_rap_phi_real[ipid]->GetBinContent( iBinPt, iBinRap, iBinPhi );
          hist_pt_rap_phi_real[ipid]->SetBinContent( iBinPt, iBinRap, iBinPhi, val+1 ); // Fill method for uneven binning
        }
        else  hist_pt_rap_phi_real[ipid]->Fill( pt, particleY[iTrack], particle_phi ); 
      }
    ...
    // Second loop for particle tracks
    for( int iTrack=0; iTrack<nTracks; iTrack++ )
    {
      ...
      // For RP correlation betwen REAL and ESTI
  ...
  for( int ipid=0; ipid<nParticleType; ipid++ )
  {
    for( int irap=0; irap<nBinRap; irap++ )
    {
      for( int ipt=0; ipt<nBinPt; ipt++ )
      {
        ...
        if( bUseOptAzi ) 
        {
          binMphi = (nBinPhy+1) - nBinPhyOpt[ipt]; // -pi bin from Center (nBinPhy+1) bin
          binPphi = (nBinPhy+1) + nBinPhyOpt[ipt]; // +pi bin from Center (nBinPhy+1) bin
        }
  ...
  if( bUseOptAzi )
  {
    // This is for the normalization of optimized azimuth binning
    for( int ipid=0; ipid<nParticleType; ipid++ )
    {
      for( int ipt=0; ipt<nBinPt; ipt++ )
      {
        double fact = fNumberOfEvents * binWidthPhyOpt[ipt] * binWidthRap * binWidthPt[ipid];
        for( int irap=0; irap<nBinRap; irap++ )
          for( int iphi=0; iphi<nBinPhi; iphi++ )
          {
            double val = hist_pt_rap_phi_meas[ipid]->GetBinContent( ipt+1, irap+1, iphi+1 );
            //double err = hist_pt_rap_phi_meas[ipid]->GetBinError( ipt+1, irap+1, iphi+1 );
            double err = TMath::Sqrt( val );

            hist_pt_rap_phi_meas[ipid]->SetBinContent( ipt+1, irap+1, iphi+1, val/fact );
            hist_pt_rap_phi_meas[ipid]->SetBinError( ipt+1, irap+1, iphi+1, err/fact );
          }
      }
    }
  }
    ...
    for( int ipt=0; ipt<nBinPt; ipt++ )
      for( int irap=0; irap<nBinRap; irap++ )
        for( int iphi=0; iphi<nBinPhi; iphi++ )
        {
          double val = hist_pt_rap_phi_meas[ipid]->GetBinContent( ipt+1, irap+1, iphi+1 );
          double err = hist_pt_rap_phi_meas[ipid]->GetBinError( ipt+1, irap+1, iphi+1 );
          double cen = hist_pt_rap_phi_meas[ipid]->GetXaxis()->GetBinCenter( ipt+1 );

          hist_pt_rap_phi_meas[ipid]->SetBinContent( ipt+1, irap+1, iphi+1, val/cen );
          hist_pt_rap_phi_meas[ipid]->SetBinError( ipt+1, irap+1, iphi+1, err/cen );
        }
  // Normalize the data
  // ------------------------------------------------------------------------------------------------------
  ...
  if( fUpdateRestored )
  {
    ...
    for( int ipt=0; ipt<nBinPt; ipt++ )
    {
      //            Centered bin    +/- pi        idx bin (compensate)
      int startBin = 0;
      int endBin   = nBinPhi - 1;
      double localBinWidthPhy = binWidthPhy;

      if( bUseOptAzi )
      {
        startBin = (nBinPhy+1) - nBinPhyOpt[ipt] - 1; // Start bin - 1 (index prob)
        endBin   = (nBinPhy+1) + nBinPhyOpt[ipt] - 1; // End bin - 1 (index prob)
        localBinWidthPhy = binWidthPhyOpt[ipt];

        // If optimized azimuth bin is used,
        // reset the central limit distribution and re-fill with corresponding binnings.
        hist_central->Reset();

        double summ = 0.;
        for( int iphi=startBin; iphi<=endBin; iphi++ ) // Consider the last bin
        {
          //double phi = hist_central->GetBinCenter( iphi+1 );
          double phi = ( iphi-nBinPhy )*localBinWidthPhy;

          double bph = facq * TMath::Cos( phi );
          double val = aph * ( 1. + TMath::Sqrt(TMath::Pi()) * bph * TMath::Exp( bph*bph ) * ( 1. + TMath::Erf(bph) ) );
          val *= localBinWidthPhy;

          hist_central->SetBinContent( iphi+1, val );
          if( iphi<endBin ) summ += val; // To avoid the last bin
        }
        hist_central->Scale( 1./summ );
      }
      ...
      {
        for( int irap=0; irap<nBinRap; irap++ )
          for( int ireal=startBin; ireal<=endBin; ireal++ )
            for( int imeas=startBin; imeas<=endBin; imeas++ )
            {
              ...
              hist_phi_diff[ipid][ipt][irap]->SetBinContent( ireal+1, imeas+1, val );
            }
      }
    }
  }
...
void SimulationTools::SampleAndMakeTM()
{
  ...
  for( int iEvent=0; iEvent<fNumberOfEvents; iEvent++ ) // loop over events
  {
    ...
    for( int iTrack=0; iTrack<nTracks; iTrack++ )
    {
      ...
      if( fModel ) 
      {
        if( bUseOptAzi )
        {
          int iBinPhi = TMath::Nint( particle_phi/binWidthPhyOpt[iBinPt-1] ) + (nBinPhy+1);
          double val = hist_pt_rap_phi_real[ipid]->GetBinContent( iBinPt, iBinRap, iBinPhi );
          hist_pt_rap_phi_real[ipid]->SetBinContent( iBinPt, iBinRap, iBinPhi, val+1 ); // Fill method for uneven binning
        }
        else hist_pt_rap_phi_real[ipid]->Fill( pt, particleY[iTrack], particle_phi ); 
    ...
    if( fUseTotalQ )
    {
      ...
      // Move pi bin -> -pi bin
      if( hist_totalq->GetXaxis()->FindBin( phiqq ) == nBinQphi ) phiqq = phiqq - TMath::TwoPi();
      if( hist_totalq->GetXaxis()->FindBin( phi_save_rp ) == nBinQphi ) phi_save_rp = phi_save_rp - TMath::TwoPi();
  ...
  // Compensating the end of the bins
  for( int ix=0; ix<hist_totalq->GetNbinsX(); ix++ )
    for( int irap=0; irap<hist_totalq->GetNbinsY(); irap++ )
    {
      double val = hist_totalq->GetBinContent( 1, irap+1 );
      hist_totalq->SetBinContent( hist_totalq->GetNbinsX(), irap+1, val );

      val = hist_totalq->GetBinContent( ix+1, 1 );
      hist_totalq->SetBinContent( ix+1, hist_totalq->GetNbinsY(), val );
    }
  // Compensating the end of the bins
...
void SimulationTools::CallParticle()
{
  ...
  else if( fHistogramRestored.size() != 0 )
  {
    for( int ipid=0; ipid<nParticleType; ipid++ )
    {
      ...
      // Implement particles' phi bin smapling
      for( auto iBin : fCheckedBin[ipid] )
      {
        ...
        {
          int ipt = binx-1;
          int iphi = binz - (nBinPhy+1); // Substract w/ centered bin (nBinPhy+1)
          phi_width = binWidthPhyOpt[ipt];
          phi_center = iphi * phi_width;
        }
...
void SimulationTools::ConstructTMfromQ()
{
  ...
  for( int ireal=0; ireal<nBinQphi-1; ireal++ )
  {
    ...
    if( content!=0. )
      for( int imeas=0; imeas<nBinQphi; imeas++ )
  ...
  for( int ipid=0; ipid<nParticleType; ipid++ )
  {
    ...
    for( int ipt=0; ipt<nBinPt; ipt++ )
    {
      ...
      for( int irap=0; irap<nBinRap; irap++ )
      {
            ...
            for( int imeas_rp=0; imeas_rp<nBinQphi-1; imeas_rp++ )
            {
              ...
              if( effi!=0. )
              {
                for( int ireal_rp=0; ireal_rp<nBinQphi-1; ireal_rp++ )
                {
                  ...
                  if( locPhiBin==nBinPhyOpt[ipt] ) locPhiBin = -nBinPhyOpt[ipt]; // Set pi bin -> -pi bin

                  //                       Centered bin
                  int phiBin = locPhiBin + (nBinPhy+1);
        ...
        {
          // X-axis compensating
          double val = hist_phi_diff[ipid][ipt][irap]->GetBinContent( iphi+1, startBin+1 );
          hist_phi_diff[ipid][ipt][irap]->SetBinContent( iphi+1, endBin+1, val );

          // Y-axis compensating
          val = hist_phi_diff[ipid][ipt][irap]->GetBinContent( startBin+1, iphi+1 );
          hist_phi_diff[ipid][ipt][irap]->SetBinContent( endBin+1, iphi+1, val );
        }
  ...
  for( int ipid=0; ipid<nParticleType; ipid++ )
    for( int ipt=0; ipt<nBinPt; ipt++ )
      for( int irap=0; irap<nBinRap; irap++ )
        for( int iphi=0; iphi<nBinPhi; iphi++ )
        {
          double err = fHistogramMeasured[ipid]->GetBinError( ipt+1, irap+1, iphi+1 );
          if( err )
          {
            double val = fHistogramMeasured[ipid]->GetBinContent( ipt+1, irap+1, iphi+1 );
            double ANS    = val/err;
            double AN     = ANS*ANS;
            double factor = val/AN;
            int multi     = gRandom->Poisson( AN );
            hist_pt_rap_phi_meas[ipid]->SetBinContent( ipt+1, irap+1, iphi+1, factor*multi );
          }
        }
}
  ...
  for( int ipid=0; ipid<nParticleType; ipid++ )
    for( int ipt=0; ipt<nBinPt; ipt++ )
      for( int irap=0; irap<nBinRap; irap++ )
        for( int iphi=0; iphi<nBinPhi; iphi++ )
        {
          if( nSample==0 ) hist_pt_rap_phi_rest[ipid]->SetBinError( ipt+1, irap+1, iphi+1, 0 );
          double err = hist_pt_rap_phi_rest[ipid]->GetBinError( ipt+1, irap+1, iphi+1 );
          double val = hist_pt_rap_phi_rest[ipid]->GetBinContent( ipt+1, irap+1, iphi+1 );
          double val_org = fHistogramRestored[ipid]->GetBinContent( ipt+1, irap+1, iphi+1 );
          if( ipid==4 ) cout << setw(4) << nSample << " " << setw(4) << ipt << " " << setw(4) << irap << " " << setw(4) << iphi << " " << setw(12) << val << " " << setw(12) << val_org << " " << setw(12) << err << endl;
          err = err*err * nSample + (val-val_org)*(val-val_org);
          err = TMath::Sqrt( err / (nSample+1) );

          hist_pt_rap_phi_rest[ipid]->SetBinError( ipt+1, irap+1, iphi+1, err );
        }
}
    ...
    for( int ipt=0; ipt<nBinPt; ipt++ )
    {
      ...
      for( int irap=0; irap<nBinRap; irap++ )
        for( int iphi=startBin; iphi<=endBin; iphi++ )
        {
          ...
          hist_pt_rap_phi_rest[ipid]->SetBinContent( ipt+1, irap+1, iphi+1, val );
        }
    }
  ...
  for( int IT=0; IT<nIteration; IT++ )
  {
    ...
    for( int ipid=0; ipid<nParticleType; ipid++ )
    {
      for( int ipt=0; ipt<nBinPt; ipt++ )
      {
        ...
        for( int irap=0; irap<nBinRap; irap++ )
        {
          ...
          {
            double nrj = 0;
            for( int ireal=startBin; ireal<=(endBin-1); ireal++ )
            {
              double lhs = hist_phi_diff[ipid][ipt][irap]->GetBinContent( ireal+1, imeas+1 );
              double rhs = hist_pt_rap_phi_rest[ipid]->GetBinContent( ipt+1, irap+1, ireal+1 );

              nrj += lhs*rhs;
            }

            // n^r_j, acting P_ji on N^r
            hist_pt_rap_phi_blur[ipid]->SetBinContent( ipt+1, irap+1, imeas+1, nrj );
          }
        }
      }
    }
      ...
      for( int ipt=0; ipt<nBinPt; ipt++ )
      {
        ...
        for( int irap=0; irap<nBinRap; irap++ )
        {
          ...
          for( int ireal=startBin; ireal<=endBin; ireal++ )
          {
            ...
            for( int imeas=startBin; imeas<=(endBin-1); imeas++ )
            {
              double lhs1 = 0;
              lhs1 = hist_pt_rap_phi_meas[ipid]->GetBinContent( ipt+1, irap+1, imeas+1 ); // Estimated
              double lhs2 = hist_pt_rap_phi_blur[ipid]->GetBinContent( ipt+1, irap+1, imeas+1 ); // One more blurred
              double lhs = 0;
              if( lhs2 != 0 ) lhs = lhs1 / lhs2;
              double rhs = hist_phi_diff[ipid][ipt][irap]->GetBinContent( ireal+1, imeas+1 );
            ...
            if( ari!=0 && weighted_prob!=0 )
            {
              ...
              if( binPhiM1 < 1 )        binPhiM1 = endBin;
              if( endBin   < binPhiP1 ) binPhiP1 = 1;

              // Switch to Pawels modulo
              // binPhiM1, binPhiP1 should be in (-nBinPhy, nBinPhy)
              binPhiM1 = modulo( ireal-nBinPhy -1+nBinPhy, 2*nBinPhy ) - nBinPhy;
              binPhiP1 = modulo( ireal-nBinPhy +1+nBinPhy, 2*nBinPhy ) - nBinPhy;
              if( bUseOptAzi )
              {
                // binPhiM1, binPhiP1 should be in (-nBinPhyOpt[ipt], nBinPhyOpt[ipt])
                binPhiM1 = modulo( ireal-nBinPhy -1+nBinPhyOpt[ipt], 2*nBinPhyOpt[ipt] ) - nBinPhyOpt[ipt];
                binPhiP1 = modulo( ireal-nBinPhy +1+nBinPhyOpt[ipt], 2*nBinPhyOpt[ipt] ) - nBinPhyOpt[ipt];
              }
              // Move the bins to (1, nBinPhi)
              ...
              // Phi regularization
              vm1 = hist_pt_rap_phi_rest[ipid]->GetBinContent( ipt+1, irap+1, binPhiM1 );
              v_0 = hist_pt_rap_phi_rest[ipid]->GetBinContent( ipt+1, irap+1, binPhi0 );
              vp1 = hist_pt_rap_phi_rest[ipid]->GetBinContent( ipt+1, irap+1, binPhiP1 );
              scaleLambda += (v_0 < vm1) ? -1 : 1;
              scaleLambda += (v_0 < vp1) ? -1 : 1;

              // rapidity regularization
              if( 1 < irap && irap < nBinRap-1 )
              {
                vm1 = hist_pt_rap_phi_rest[ipid]->GetBinContent( ipt+1, irap+1 -1, binPhi0 );
                v_0 = hist_pt_rap_phi_rest[ipid]->GetBinContent( ipt+1, irap+1   , binPhi0 );
                vp1 = hist_pt_rap_phi_rest[ipid]->GetBinContent( ipt+1, irap+1 +1, binPhi0 );
              ...
              if( ipt < (nBinPt-1) )
              {
                ...
                if( bUseOptAzi )
                {
                  double phi = (ireal-nBinPhy) * binWidthPhyOpt[ipt];
                  binPhiF = TMath::Nint( phi/binWidthPhyOpt[ipt+1] );
                  binPhiF = binPhiF + nBinPhy + 1;
                }
                vp1 = hist_pt_rap_phi_rest[ipid]->GetBinContent( ipt+1 +1, irap+1, binPhiF );
                scaleLambda += (v_0 < vp1) ? -0.5 : 0.5;
                ...
                if( 0 < ipt )
                {
                  ...
                  {
                    double phi = (ireal-nBinPhy) * binWidthPhyOpt[ipt];
                    binPhiB = TMath::Nint( phi/binWidthPhyOpt[ipt-1] );
                    binPhiB = binPhiB + nBinPhy + 1;
                  }
                  vm1 = hist_pt_rap_phi_rest[ipid]->GetBinContent( ipt+1 -1, irap+1, binPhiB );
                  scaleLambda += (v_0 < vm1) ? -0.5 : 0.5;
                }
                else
                {
                  ...
                  if( bUseOptAzi ) thisPhiBin = modulo( binComp, nBinPhyOpt[ipt]*2 ) - nBinPhyOpt[ipt];
                  thisPhiBin = thisPhiBin + nBinPhy + 1;
                  vm1 = hist_pt_rap_phi_rest[ipid]->GetBinContent( ipt+1, irap+1, thisPhiBin );
                  scaleLambda += (v_0 < vm1) ? -0.5 : 0.5;
                }
              }
              ...
              if( reg_on ) 
              {
                ...
                //     phi=0             phi=-pi            phi=pi
                if( ireal==nBinPhy || ireal==startBin || ireal==endBin ) regul = 1. / (1. + 0.25*scaleLambda*fLambda);
              }
              ...
              hist_pt_rap_phi_temp[ipid]->SetBinContent( ipt+1, irap+1, ireal+1, final_val );
            }
          }
    ...
    for( int ipid=0; ipid<nParticleType; ipid++ )
    {
      for( int ipt=0; ipt<nBinPt; ipt++ )
      {
        ...
        for( int irap=0; irap<nBinRap; irap++ )
          for( int i=startBin; i<=endBin; i++ ) hist_pt_rap_phi_rest[ipid]->SetBinContent( ipt+1, irap+1, i+1, hist_pt_rap_phi_temp[ipid]->GetBinContent(ipt+1, irap+1, i+1) );
      }
    }
...
{
  std::vector< TH3D* > localHistogramMeasured;
  std::vector< TH3D* > localHistogramRestored;

  for( int ipid=0; ipid<nParticleType; ipid++ )
  {
    auto local_hist_meas = (TH3D*) hist_pt_rap_phi_meas[ipid]->Clone( Form("clone_meas_%s", particleName[ipid].Data()) );
    auto local_hist_rest = (TH3D*) hist_pt_rap_phi_rest[ipid]->Clone( Form("clone_rest_%s", particleName[ipid].Data()) );
    localHistogramMeasured.push_back( local_hist_meas );
    localHistogramRestored.push_back( local_hist_rest );
  }


  double localBinWidthPhy = binWidthPhy; // Keep this...
  ...
  for( int ipid=0; ipid<nParticleType; ipid++ )
  {
    for( int irap=0; irap<nBinRap; irap++ )
      ...
      {
        int ipt_wipe = 0;
        double val_bk = localHistogramRestored[ipid]->GetBinContent( ipt_start+1, irap+1, iphi+1 );
        for( int ipt=ipt_start+1; ipt<nBinPt; ipt++ )
        {
          double val_inc = localHistogramRestored[ipid]->GetBinContent( ipt+1, irap+1, iphi+1 );
          if( 3*val_bk < val_inc ) { ipt_wipe=ipt; break; }
          else val_bk = val_inc;
        }

        for( int ipt=ipt_wipe; ipt_wipe && ipt<nBinPt; ipt++ )
          localHistogramRestored[ipid]->SetBinContent( ipt+1, irap+1, iphi+1, 0. );
      }
  }
  ...
  for( int ipid=0; ipid<nParticleType; ipid++ ) 
  {
    ...
    for( int irap=0; irap<nBinRap; irap++ )
    {
      ...
      for( int ipt=0; ipt<nBinPt; ipt++ )
      {
        double pt = localHistogramRestored[ipid]->GetXaxis()->GetBinCenter( ipt+1 );
        ...
        for( int iphi=startBin; iphi<endBin; iphi++ ) // The both end bins are the same bin
        {
          double phi = localHistogramRestored[ipid]->GetZaxis()->GetBinCenter( iphi+1 );
          double val = localHistogramRestored[ipid]->GetBinContent( ipt+1, irap+1, iphi+1 );
          if( dnCut < val )
          {
            double phif = double( iphi-nBinPhy+0.5 ) * localBinWidthPhy;
            double phib = double( iphi-nBinPhy-0.5 ) * localBinWidthPhy;
            double cosint = TMath::Sin( phif ) - TMath::Sin( phib );
            //if( ipid==4 && irap==13 ) cout << ipt+1 << " " << irap-20 << " " << iphi-12 << " " << val << endl;

            dnSumPhi = dnSumPhi + val;
            dphiSum  = dphiSum + val*cosint;

            double measVal = localHistogramMeasured[ipid]->GetBinContent( ipt+1, irap+1, iphi+1 );
            dnSumPhi_meas = dnSumPhi_meas + measVal;
          }
        }

        dnSum   = dnSum  + dnSumPhi*localBinWidthPhy*fapd;;
        dpxSum  = dpxSum + dphiSum*fapdpt;
        dpt2Sum = dpt2Sum + dnSumPhi*localBinWidthPhy*fapdptk;

        // For cut (avg)
        dnSum_meas = dnSum_meas + dnSumPhi_meas*localBinWidthPhy*fapd;
      }
      ...
      if( dnSum )
      {
        ...
        {
          double wgr = pxr / den;
          optWeight[ipid][irap] = wgr;
        }
      }

      if( 0<=(irap-20) && (irap-20)<=rapHalfBin/2 ) totalSum = totalSum + dnSum; // Only mid and fwd RAP
      dnSumMeas[irap] = dnSum_meas;
    }

    double avgSum = totalSum / ( rapHalfBin/2+1 );
    double avgCut = avgSum / 40.; // cut suppressing suspect low intensity y-regions

    for( int irap=0; irap<nBinRap; irap++ )
      if( dnSumMeas[irap] < avgCut ) optWeight[ipid][irap] = 0;

    for( int irap=0; irap<nBinRap; irap++ ) cout << ipid << " " << irap << " " << optWeight[ipid][irap] << endl;
  }
...
void SimulationTools::SetMeasuredHist( std::vector<TH3D*> &histMeasured )
{
  for( int ipid=0; ipid<nParticleType; ipid++ )
  {
    auto local_hist_meas = (TH3D*) histMeasured[ipid]->Clone( Form("clone_meas_%s", particleName[ipid].Data()) );
    fHistogramMeasured.push_back( local_hist_meas );
  }
}

void SimulationTools::SetRestoredHist( std::vector<TH3D*> &histRestored )
{
  for( int ipid=0; ipid<nParticleType; ipid++ )
  {
    auto local_hist_rest = (TH3D*) histRestored[ipid]->Clone( Form("clone_rest_%s", particleName[ipid].Data()) );
    fHistogramRestored.push_back( local_hist_rest );
  }
}

void SimulationTools::ResetRestoredHist()
{
  for( int ipid=0; ipid<nParticleType; ipid++ )
    for( int ipt=0; ipt<nBinPt; ipt++ )
      for( int irap=0; irap<nBinRap; irap++ )
        for( int iphi=0; iphi<nBinPhi; iphi++ )
        {
          double val_org = fHistogramRestored[ipid]->GetBinContent( ipt+1, irap+1, iphi+1 ); // Original distribution
          double err = hist_pt_rap_phi_rest[ipid]->GetBinError( ipt+1, irap+1, iphi+1 ); // Keep this error

          hist_pt_rap_phi_rest[ipid]->SetBinContent( ipt+1, irap+1, iphi+1, val_org );
          hist_pt_rap_phi_rest[ipid]->SetBinError( ipt+1, irap+1, iphi+1, err );
        }
}
